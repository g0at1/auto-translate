...
class AddDialog(simpledialog.Dialog):
    def body(self, master):
        ttk.Label(master, text='Key path (dot separated):').grid(row=0, column=0, sticky='w')
...
class TranslationApp(tk.Tk):
    def __init__(self, pl_path, en_path):
        ...
        ttk.Button(btn_frame, text='Add New', command=self.add_new).pack(side='left', padx=5, pady=5)
    ...
    def finish_insert(self, parts, en_text):
        if not en_text:
            en_text = simpledialog.askstring(
                'English translation',
                'Enter English text:'
            ) or ''
